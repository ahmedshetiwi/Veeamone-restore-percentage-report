...
param(
    ...
    # Retry-aware success: if a restore of the SAME object failed but a later
    # attempt SUCCEEDED within this many hours, count the failed attempt as
    # success (it was resolved on retry). Set 0 to disable. Default 6 hours.
    [int]      $RetryWindowHours = 6,

    # Count by restore JOB (session) instead of per restored object: all objects
    # of one restore session collapse to a single row (job status = worst object
    # outcome, size = sum).
    [switch]   $ByRestoreJob,
...
if ($DemoData) {
    ...
    for ($d = 0; $d -lt $spanDays; $d++) {
        ...
        foreach ($i in 0..($rng.Next(1,5))) {
            ...
            # A restore job (session) can contain several objects; give this day a
            # small pool of session ids so multiple objects share a job.
            $sid = 'demo-sess-{0}-{1}' -f $d, $rng.Next(0,2)
    ...
    # Deterministic retry case: same object fails, then succeeds 2h later (only
    # added when it falls inside the selected range).
    $retryDay = (Get-Date).Date.AddDays(-3)
    $f1 = $retryDay.AddHours(1); $f1e = $f1.AddMinutes(4)
    $s1 = $retryDay.AddHours(3); $s1e = $s1.AddMinutes(52)
    if ($f1 -ge $StartDate -and $s1 -lt $EndDate) {
        $records.Add([pscustomobject]@{ RestoreType='File-Level Restore'; ObjectName='C:\NASTEST'; DestinationHost='FILESRV01'; Status='Failed';  StartDt=$f1; EndDt=$f1e; SessionUid='demo-retry-fail'; SizeBytesNum=0; StartDate=$f1.ToString('yyyy-MM-dd'); StartTime=$f1.ToString('HH:mm:ss'); EndDate=$f1e.ToString('yyyy-MM-dd'); EndTime=$f1e.ToString('HH:mm:ss'); ElapsedTime=(Format-Elapsed -Start $f1 -End $f1e); Size=''; Reason='Restore failed: network path not found (demo)' }) | Out-Null
        $records.Add([pscustomobject]@{ RestoreType='File-Level Restore'; ObjectName='C:\NASTEST'; DestinationHost='FILESRV01'; Status='Success'; StartDt=$s1; EndDt=$s1e; SessionUid='demo-retry-ok'; SizeBytesNum=[long](250MB); StartDate=$s1.ToString('yyyy-MM-dd'); StartTime=$s1.ToString('HH:mm:ss'); EndDate=$s1e.ToString('yyyy-MM-dd'); EndTime=$s1e.ToString('HH:mm:ss'); ElapsedTime=(Format-Elapsed -Start $s1 -End $s1e); Size=(Format-Size (250MB)); Reason='' }) | Out-Null
    }
    ...
    foreach ($s in $sessions.Rows) {
        ...
        $records.Add([pscustomobject]@{
            ...
            StartDt         = $start
            EndDt           = $end
            SessionUid      = if ($s.Table.Columns.Contains('SessionUid') -and $s['SessionUid'] -isnot [DBNull]) { [string]$s['SessionUid'] } else { '' }
            SizeBytesNum    = $(if ($s['SizeBytes'] -isnot [DBNull]) { $z=0.0; [double]::TryParse([string]$s['SizeBytes'],[ref]$z)|Out-Null; [long]$z } else { 0 })
...
# ---------------------------------------------------------------------------
# 7b. Retry-aware success: a restore of the SAME object that failed but then
#     SUCCEEDED again within $RetryWindowHours is counted as Success (it was
#     resolved on the next retry). The failed attempt's row is re-marked Success
#     and its Reason notes the retry, so it no longer counts as a failure.
# ---------------------------------------------------------------------------
$retriedCount = 0
if ($RetryWindowHours -gt 0) {
    $successTimesByKey = @{}
    foreach ($r in $records) {
        if ($r.Status -eq 'Success') {
            $k = ("{0}|{1}|{2}" -f $r.ObjectName, $r.DestinationHost, $r.RestoreType).ToLower()
            if (-not $successTimesByKey.ContainsKey($k)) { $successTimesByKey[$k] = New-Object System.Collections.Generic.List[datetime] }
            [void]$successTimesByKey[$k].Add($r.StartDt)
        }
    }
    $window = [TimeSpan]::FromHours($RetryWindowHours)
    foreach ($r in $records) {
        if ($r.Status -ne 'Failed') { continue }
        $k = ("{0}|{1}|{2}" -f $r.ObjectName, $r.DestinationHost, $r.RestoreType).ToLower()
        if (-not $successTimesByKey.ContainsKey($k)) { continue }
        $laterOk = @($successTimesByKey[$k] | Where-Object { $_ -gt $r.StartDt -and ($_ - $r.StartDt) -le $window } | Sort-Object)
        if ($laterOk.Count -gt 0) {
            $when = $laterOk[0].ToString('yyyy-MM-dd HH:mm')
            $note = "Auto-resolved: initially failed, succeeded on retry at $when"
            $r.Reason  = if ($r.Reason) { "$note (was: $($r.Reason))" } else { $note }
            $r.Status  = 'Success'
            $retriedCount++
        }
    }
    if ($retriedCount -gt 0) {
        Write-Host "Retry-aware: $retriedCount failed restore(s) succeeded on retry within $RetryWindowHours h -> counted as success." -ForegroundColor Cyan
    }
}

# ---------------------------------------------------------------------------
# 7c. By restore JOB: collapse all objects of one restore session into a single
#     row (job status = worst object outcome; size = sum; times = span).
# ---------------------------------------------------------------------------
$unitLabel = if ($ByRestoreJob) { 'restore jobs' } else { 'restores' }
if ($ByRestoreJob) {
    function Get-JobStatus {
        param($Items)
        $names = @($Items | ForEach-Object Status)
        if ($names -contains 'Failed')  { return 'Failed' }
        if ($names -contains 'Running')  { return 'Running' }
        if ($names -contains 'Warning')  { return 'Warning' }
        if ($names -contains 'Success')  { return 'Success' }
        if ($names -contains 'Skipped')  { return 'Skipped' }
        return 'Unknown'
    }
    $keyed = foreach ($r in $records) {
        $k = if ($r.SessionUid) { $r.SessionUid } else { [guid]::NewGuid().ToString() }
        [pscustomobject]@{ K = $k; R = $r }
    }
    $jobRecords = New-Object System.Collections.Generic.List[object]
    foreach ($grp in ($keyed | Group-Object K)) {
        $items = @($grp.Group | ForEach-Object R)
        $st    = Get-JobStatus $items
        $minStart = ($items | Sort-Object StartDt | Select-Object -First 1).StartDt
        $endItems = @($items | Where-Object { $_.EndDt })
        $maxEnd   = if ($endItems.Count -gt 0) { ($endItems | Sort-Object EndDt | Select-Object -Last 1).EndDt } else { $null }
        $objNames = @($items | ForEach-Object ObjectName | Where-Object { $_ } | Select-Object -Unique)
        $hosts    = @($items | ForEach-Object DestinationHost | Where-Object { $_ } | Select-Object -Unique)
        $objCell  = if ($objNames.Count -le 1) { ($objNames | Select-Object -First 1) } else { "$($objNames.Count) objects" }
        $hostCell = if ($hosts.Count -le 1) { ($hosts | Select-Object -First 1) } else { "$($hosts.Count) hosts" }
        $typeCell = ($items | Group-Object RestoreType | Sort-Object Count -Descending | Select-Object -First 1).Name
        $sumBytes = ($items | Measure-Object -Property SizeBytesNum -Sum).Sum; if (-not $sumBytes) { $sumBytes = 0 }
        $failReason = @($items | Where-Object { $_.Status -eq 'Failed' -and $_.Reason } | ForEach-Object Reason | Select-Object -First 1)
        $reason = if ($items.Count -gt 1) { "$($items.Count) object(s)$(if ($failReason) { " - $failReason" })" } elseif ($failReason) { $failReason } else { ($items | Select-Object -First 1).Reason }
        $jobRecords.Add([pscustomobject]@{
            RestoreType     = $typeCell
            ObjectName      = $objCell
            DestinationHost = $hostCell
            Status          = $st
            StartDt         = $minStart
            StartDate       = $minStart.ToString('yyyy-MM-dd')
            StartTime       = $minStart.ToString('HH:mm:ss')
            EndDate         = if ($maxEnd) { $maxEnd.ToString('yyyy-MM-dd') } else { '' }
            EndTime         = if ($maxEnd) { $maxEnd.ToString('HH:mm:ss') } else { '' }
            ElapsedTime     = if ($maxEnd) { Format-Elapsed -Start $minStart -End $maxEnd } else { '' }
            Size            = Format-Size $sumBytes
            Reason          = $reason
        }) | Out-Null
    }
    $records = $jobRecords
    Write-Host "By restore job: collapsed to $($records.Count) restore job(s) (from restored objects)." -ForegroundColor Cyan
}
...
<div class='wrap'>
  <header>
    <div>
      ...
      <div class='sub'>Restore success / failure rate &middot; counted by $(if ($ByRestoreJob) { 'restore job (session)' } else { 'restored object' }) &middot; $rangeText &middot; DB: $(Get-Html $connInfo.Database)</div>
    </div>
    <div class='sub'>Generated $(Get-Date -Format 'yyyy-MM-dd HH:mm')</div>
  </header>

  <div class='cards'>
    <div class='card'><div class='n'>$total</div><div class='l'>Total $unitLabel</div></div>
    ...
    $(if ($retriedCount -gt 0) { "<div class='card'><div class='n' style='color:$($statusColors.Success)'>$retriedCount</div><div class='l'>Resolved on retry</div></div>" })
    <div class='card'><div class='n'>$rate%</div><div class='l'>Success rate</div></div>
  </div>
  $(if ($retriedCount -gt 0) { "<div class='sub' style='margin:-6px 0 10px;'>$retriedCount failed restore(s) that succeeded again within $RetryWindowHours h are counted as success.</div>" })
