...
param(
    ...
    # Which report to schedule. All reports share this scheduler + the same
    # e-mail settings. Default 'Restore' keeps the original behaviour.
    [ValidateSet('Restore','BackupJobs','RmanBackups','AllBackups','SapHana')]
    [string]   $ReportType = 'Restore',

    # Generic filter passed to the chosen report:
    #   Restore     -> -RestoreType   (restore item type LIKE)
    #   RmanBackups -> -JobNameLike   (job name LIKE, default %RMAN%)
    #   BackupJobs  -> -NameLike      (job name LIKE)
    #   AllBackups  -> -NameLike      (job name LIKE)
    [string]   $Filter,
...
# Map the report type -> its engine script, filter parameter, and default task name.
$reportMap = @{
    Restore     = @{ Script = 'Get-VeeamOneRestoreReport.ps1';   FilterParam = 'RestoreType'; TaskName = 'VeeamONE Restore Report' }
    BackupJobs  = @{ Script = 'Get-VeeamOneBackupJobReport.ps1'; FilterParam = 'NameLike';    TaskName = 'VeeamONE Backup Jobs Report' }
    RmanBackups = @{ Script = 'Get-VeeamOneRmanBackupReport.ps1';FilterParam = 'JobNameLike'; TaskName = 'VeeamONE RMAN Backup Report' }
    AllBackups  = @{ Script = 'Get-VeeamOneAllBackupsReport.ps1';FilterParam = 'NameLike';    TaskName = 'VeeamONE All Backups Report' }
    SapHana     = @{ Script = 'Get-VeeamOneSapHanaReport.ps1';   FilterParam = 'JobNameLike'; TaskName = 'VeeamONE SAP HANA Report' }
}
$rp = $reportMap[$ReportType]
if (-not $TaskName) { $TaskName = $rp.TaskName }

$reportScript = Join-Path $PSScriptRoot $rp.Script
if (-not $Unregister -and -not (Test-Path $reportScript)) {
    throw "Cannot find $($rp.Script) next to this script ($reportScript)."
}
...
# Filter: prefer the generic -Filter; fall back to -RestoreType for back-compat.
$filterValue = if ($Filter) { $Filter } elseif ($RestoreType) { $RestoreType -join ',' } else { $null }
if ($filterValue) { [void]$sb.Append(('-{0} "{1}" ' -f $rp.FilterParam, $filterValue)) }
...
# --- Define the task ---
$task = $svc.NewTask(0)
$task.RegistrationInfo.Description = "Runs the $($rp.TaskName) ($Cadence) and e-mails it."
